...
def map_simplex_equiv {n m : Type*} [Fintype n] [Fintype m] (e : n ≃ m) :
  ...
  left_inv x := by
    apply Subtype.ext
    funext i
    show x.1 (e.symm ((e.symm).symm i)) = x.1 i
    rw [Equiv.symm_symm, Equiv.symm_apply_apply]
  right_inv x := by
    apply Subtype.ext
    funext i
    show x.1 ((e.symm).symm (e.symm i)) = x.1 i
...
def map_mixedS_equiv {G : FinGame} (e : (i : G.I) → G.SS i ≃ Fin (Fintype.card (G.SS i))) :
  ...
  left_inv x := by
    ...
    apply Subtype.ext
    funext j
    show (x i).1 ((e i).symm (((e i).symm).symm j)) = (x i).1 j
    rw [Equiv.symm_symm, Equiv.symm_apply_apply]
  right_inv x := by
    funext i
    apply Subtype.ext
    funext j
    show (x i).1 (((e i).symm).symm ((e i).symm j)) = (x i).1 j
    rw [Equiv.symm_symm, Equiv.apply_symm_apply]
...
theorem Brouwer.mixedGame (f : G.mixedS → G.mixedS) (hf : Continuous f) : ∃ x : G.mixedS, f x = x := by
  ...
  have map_idx_left : ∀ y, map_idx_inv (map_idx y) = y := by
    ...
    apply Subtype.ext
    funext j
    show (y k).1 ((eS k).symm (((eS k).symm).symm j)) = (y k).1 j
    rw [Equiv.symm_symm, Equiv.symm_apply_apply]
  have map_idx_right : ∀ z, map_idx (map_idx_inv z) = z := by
    intro z; funext k
    apply Subtype.ext
    funext j
    show (z k).1 (((eS k).symm).symm ((eS k).symm j)) = (z k).1 j
    rw [Equiv.symm_symm, Equiv.apply_symm_apply]
