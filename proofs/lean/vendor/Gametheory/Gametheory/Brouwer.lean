...
instance TT.funlike : FunLike (TT n l) (Fin n) (Fin (l+1)) where
  coe := fun a => a.1
  coe_injective' := fun _ _ h => Subtype.ext (funext fun i => congr_fun h i)
...
instance TT.IST : IsStrictTotalOrder (TT n l) (TT.Ilt i) where
  trichotomous := fun a b hab hba => by
    rcases lt_trichotomy (toLex (a i, a)) (toLex (b i, b)) with h | h | h
    · rw [TT.Ilt] at hab; exact absurd h hab
    · have hpair : (a i, a) = (b i, b) := toLex_inj.mp h
      exact (Prod.mk.inj hpair).2
    · rw [TT.Ilt] at hba; exact absurd h hba
  irrefl := fun a h => by
    rw [TT.Ilt] at h
    exact lt_irrefl _ h
  trans := fun a b c hab hbc => by
    rw [TT.Ilt] at hab hbc ⊢
...
(h2 : σ.Nonempty):
  ...
  have h_exists_point : ∃ M : TT n l, ∀ k ∈ C, m k + 1 ≤ M k := by
    ...
    · rw [h_is_zero] at hk_in_C ⊢
      show m 0 + 1 ≤ (M_val 0).1
      show m 0 + 1 ≤ M_coords 0
      show m 0 + 1 ≤ if (0 : Fin n) = 0 then M' 0 + R else M' 0
      rw [if_pos rfl]
      show m 0 + 1 ≤ (if (0 : Fin n) ∈ C then m 0 + 1 else 0) + R
      rw [if_pos hk_in_C]
      omega
    · show m k + 1 ≤ (M_val k).1
      show m k + 1 ≤ M_coords k
      show m k + 1 ≤ if k = 0 then M' 0 + R else M' k
      rw [if_neg h_is_zero]
      show m k + 1 ≤ if k ∈ C then m k + 1 else 0
      rw [if_pos hk_in_C]
  ...
  have h_not_dominant : ¬ TT.ILO.isDominant σ C := by
    intro h_dom
    rcases h_dom M with ⟨k, hk_in_C, h_M_le⟩
    rcases h_contradiction k hk_in_C with ⟨x, hx, hlt⟩
    letI : LinearOrder (TT n l) := IndexedLOrder.IST k
    exact (lt_iff_not_ge.mp hlt) (h_M_le x hx)
  ...
  · intro x hx y hy i
    ...
    have h_abs_lt_2_card : abs ((x i : ℤ) - (y i : ℤ)) < 2 * (C.card : ℤ) := by
      have h_bound_int : ∀ z ∈ σ, (z i : ℤ) - (m' i : ℤ) < C.card := by
        ...
        · have h_inner : (z i : ℕ) - m i < C.card := this
          simp
          have h_le : m i ≤ (z i : ℕ) := by
            apply Finset.min'_le
            apply Finset.mem_image_of_mem
            exact hz
          have h_eq : (((z i : ℕ) - m i : ℕ) : ℤ) = ((z i : ℕ) : ℤ) - (m i : ℤ) :=
            Nat.cast_sub h_le
          have h_lt : (((z i : ℕ) - m i : ℕ) : ℤ) < (C.card : ℤ) := by exact_mod_cast h_inner
          omega
...
theorem exists_subseq_constant_of_finite_image {s : Finset α} (e : ℕ → α) (he : ∀ n, e n ∈ s ) :
  ...
  have imgs_nonempty : imgs.Nonempty := by
    ...
    have preimages_all_finite : ∀ a ∈ s, Set.Finite (e ⁻¹' {a}) := by
      intro a ha
      by_contra hnf
      have a_in_imgs : a ∈ imgs := Finset.mem_filter.mpr ⟨ha, hnf⟩
  ...
  have h_diam_bounded : ∃ (C : ℝ), ∀ k, Metric.diam ((projected_σ k).toSet) ≤ C / (l k + 1) := by
    ...
    have h_dist_bound : ∀ x ∈ (σ k), ∀ y ∈ (σ k),
      ...
      _ ≤ 2 * Real.sqrt (n : ℝ) * ((n : ℝ) + 1) / (l k + 1) := by
          ...
          rw [h_assoc, mul_le_mul_iff_of_pos_left (by positivity : (0:ℝ) < 2)]
